...
export default function PatientDetailScreen() {
  ...
  const loadData = async (patientId: string) => {
    ...
    try {
      // User profile
      const userSnap = await getDoc(userDoc(patientId));
      if (userSnap.exists()) setPatient(userSnap.data() as User);

      // Last 20 symptoms
      const symQ = query(symptomsCol(patientId) as never, orderBy('date', 'desc'), limit(20));
      const symSnap = await getDocs(symQ);
      setSymptoms(symSnap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<SymptomEntry, 'id'>) } as SymptomEntry)));


      // Last 10 reports
      const repQ = query(reportsCol(patientId) as never, orderBy('uploadedAt', 'desc'), limit(10));
      const repSnap = await getDocs(repQ);
      setReports(repSnap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<Report, 'id'>) } as Report)));


      // Gamification
      const gamSnap = await getDoc(gamificationDoc(patientId));
      if (gamSnap.exists()) setGamification(gamSnap.data() as GamificationProfile);

      // Today's AI insight
      const todayRef = aiInsightDoc(patientId, todayDateString());
      const insightSnap = await getDoc(todayRef);
      if (insightSnap.exists()) setAiInsight(insightSnap.data() as AIInsight);
    } catch (error) {
